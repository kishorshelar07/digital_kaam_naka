/**
 * ================================================================
 * pages/WorkerDashboard.jsx — Worker's Home Base
...
const WorkerDashboard = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      // Load worker profile + recent bookings in parallel
      const profileRes = await workerService.getById(user.workerProfile?.id);
      ...
      if (user.workerProfile?.id) {
        const earningsRes = await workerService.getEarnings(user.workerProfile.id);
        if (earningsRes.data.success) setEarnings(earningsRes.data.data);
      }
    } catch (err) {
      toast.error('डेटा लोड होऊ शकला नाही');
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  // ── THE CORE FEATURE: Availability Toggle ──────────────────
const handleAvailabilityToggle = async () => {
  ...
  if (newValue === true) {
    ...
    setToggling(true);
    try {
      await workerService.setAvailability({
        isAvailable: false,
...
const confirmAvailability = async () => {
  ...
  const handleAccept = async (bookingId) => {
    try {
      ...
      if (data.success) {
        toast.success('✅ Booking स्वीकारली! मालकाला कळवले.');
        loadDashboardData();
      }
...
<div className="card card-body" style={{
  ...
  <p style={{ fontSize: 13, color: 'var(--color-text-muted)', marginBottom: 20 }}>
    {isAvailable
      ? `Employers तुम्हाला ${radius}km परिसरात पाहू शकतात`
      : 'Toggle ON करा — Employers तुम्हाला शोधतील'}
  </p>

  <button
    onClick={handleAvailabilityToggle}
    disabled={toggling || gettingLocation}
    style={{
      width: 120, height: 56, borderRadius: 28, border: 'none',
      cursor: toggling || gettingLocation ? 'wait' : 'pointer',
      background: isAvailable ? '#22c55e' : '#d1d5db',
      position: 'relative', transition: 'background 0.3s', margin: '0 auto', display: 'block',
    }}>
    <div style={{
      width: 48, height: 48, borderRadius: '50%', background: 'white',
      position: 'absolute', top: 4, left: isAvailable ? 68 : 4,
      transition: 'left 0.3s', display: 'flex', alignItems: 'center',
  ...
  <div style={{ marginTop: 14, fontSize: 13, color: 'var(--color-text-muted)' }}>
    {gettingLocation ? '📍 Location detect होत आहे...'
      : isAvailable ? `₹${parseInt(worker?.dailyRate||0).toLocaleString()}/दिवस • ${radius}km radius`
      : 'Toggle दाबा'}
  </div>
</div>
  ...
  <div style={{
    ...
    <div className="card card-body" style={{ width: '100%', maxWidth: 480, borderRadius: '20px 20px 16px 16px', padding: 24 }}>
      ...
      <div style={{
        ...
        background: detectedLocation ? '#f0fdf4' : '#fff7ed',
        border: `1px solid ${detectedLocation ? '#22c55e' : '#f97316'}`,
      }}>
        {detectedLocation ? (
          <>
            <div style={{ fontWeight: 700, color: '#16a34a', fontSize: 14 }}>✅ GPS Location मिळाली</div>
          ...
          <>
            <div style={{ fontWeight: 700, color: '#ea580c', fontSize: 14 }}>⚠️ GPS Location नाही</div>
            <div style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 }}>
              District नुसार Employers तुम्हाला शोधू शकतात
            </div>
          </>
        )}
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ textAlign: 'center', fontSize: 12, color: 'var(--color-text-muted)', marginTop: 6 }}>
          {radius <= 10 ? '🏘️ फक्त जवळचे काम' : radius <= 30 ? '🚌 बस/रिक्षाने जाता येईल' : '🚗 गाडीने जावे लागेल'}
        </div>
      </div>

      <div style={{ display: 'flex', gap: 10 }}>
        <button className="btn btn-light" style={{ flex: 1 }}
          onClick={() => setShowAvailModal(false)}>रद्द</button>
        <button className="btn btn-success" style={{ flex: 2 }}
          onClick={confirmAvailability} disabled={submitting}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {bookings.length === 0 ? (
          <div className="card card-body" style={{ textAlign: 'center', color: 'var(--color-text-muted)', padding: '32px 16px' }}>
            ...
            {!isAvailable && (
              <p style={{ fontSize: 13, marginTop: 8, color: 'var(--color-primary)', fontWeight: 600 }}>
                💡 "उपलब्ध व्हा" चा बटण दाबा — booking येईल!
              </p>
            )}
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {bookings.map(booking => (
              <div key={booking.id} className="card card-body">
                {/* Employer Info */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  <img
                    src={booking.employer?.user?.profilePhoto ||
                      `https://ui-avatars.com/api/?name=${booking.employer?.user?.name || 'E'}&background=1E3A5F&color=fff&size=48`}
                    alt=""
                    style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }}
                  />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: 16 }}>{booking.employer?.user?.name}</div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {booking.status === 'pending' && (
                    <>
                      <button onClick={() => handleAccept(booking.id)} className="btn btn-success" style={{ flex: 1 }}>
                        ✅ स्वीकारा
                      </button>
                      <button onClick={() => handleReject(booking.id)} className="btn btn-danger btn-sm" style={{ minWidth: 80 }}>
                        ❌ नाकारा
                      </button>
                    </>
                  )}
                  {booking.status === 'accepted' && (
                    <button onClick={() => handleStartWork(booking.id)} className="btn btn-primary" style={{ flex: 1 }}>
                      🔨 काम सुरू झाले
                    </button>
                  )}
                  {booking.status === 'started' && (
                    <div style={{ flex: 1, textAlign: 'center', color: 'var(--color-primary)', fontWeight: 600 }}>
                      🔨 काम सुरू आहे...
                    </div>
                  )}
                  <Link to={`/bookings/${booking.id}`} className="btn btn-light btn-sm" style={{ textDecoration: 'none' }}>
                    तपशील
                  </Link>
                </div>
              </div>
