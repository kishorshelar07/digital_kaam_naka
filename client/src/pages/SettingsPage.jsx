...
const SettingsPage = () => {
  ...
  const LINKS = [
    ...
    { icon: '👤', label: 'प्रोफाइल बदला', to: user?.role === 'worker' ? `/workers/${user?.workerProfile?._id}` : '/employer/dashboard' },
  ];
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        {LINKS.map(item => (
          <button key={item.label} onClick={() => navigate(item.to)}
            style={{ display: 'flex', alignItems: 'center', gap: 14, width: '100%', padding: '14px 20px', border: 'none', background: 'none', cursor: 'pointer', fontFamily: 'var(--font-family)', fontSize: 15, borderBottom: '1px solid var(--color-border)' }}>
      ...
      <button onClick={handleLogout} className="btn btn-danger btn-block btn-lg">🚪 {t('nav.logout')}</button>
    </div>
  );
};
